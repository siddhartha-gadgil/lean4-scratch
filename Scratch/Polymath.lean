...
def ProvedBound.conj(x: Letter)(ys fst snd: Word)(eqn : ys = fst ++ [x⁻¹] ++ snd) :
...
instance {w: Word} : Inhabited (ProvedBound w) :=
  ⟨easyBound w⟩
...
partial def provedBound : (w: Word) → ProvedBound w := fun w =>
  ...
  | x :: ys =>
    ...
    let tail := splits.map (fun ps => 
      ProvedBound.conj x ys ps.fst ps.snd ps.proof (provedBound ps.fst)
          (provedBound ps.snd))
